...
- Added an executable `mobile-evidence.js` runner that automatically invokes the routed `mobile-evidence` role before final verification, requires Appium MCP, derives Android/iOS targets, and accepts only fresh `device/mobile-device-qc.md` plus `device/screenshots/*.png` generated by the current attempt.
