---
description: Independent review of the current diff by the specialist subagents (security, code, performance, plus platform experts when their folder is touched); findings with severity and file:line, then a triage table. Usage — /review [PROJ-123 or a note about what changed]
---
...
2. Decide the reviewers: `security-reviewer` (mode `diff-review`) and
   ...
   screens, lists, hooks/queries, or startup code changed; `android-expert` /
   `ios-expert` in review mode when `android/` or `ios/` changed.
