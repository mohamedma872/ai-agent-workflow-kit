 ...
 *
 * Commands:
 *   node ai/workflow/router.js list
 *   node ai/workflow/router.js show feature [--json]
 *   node ai/workflow/router.js resolve feature implementation [--agent codex] [--json]
 *   node ai/workflow/router.js exec feature implementation --prompt-file /path/context.md
 *        [--agent NAME] [--output-file FILE] [--budget USD] [--cwd DIR] [--dry-run]
...
function readYaml(file) {
  ...
  catch (e) { throw new Error(`${path.relative(ROOT, file)}: ${e.message}`); }
}
...
function loadAgents() {
  if (!fs.existsSync(AGENTS_FILE)) { throw new Error('ai/agents.yaml is missing'); }
  return readYaml(AGENTS_FILE);
}

function stageFor(workflow, id) {
  return workflow.stages.find(s => s && s.id === id) || null;
}

function roleCandidates(role, override) {
  if (override) { return [override]; }
  const out = [];
  if (role.executor) { out.push(role.executor); }
  for (const x of role.fallback || []) { if (!out.includes(x)) { out.push(x); } }
  return out;
}

function onPath(bin) {
  if (!bin) { return false; }
  return spawnSync('which', [String(bin)], { stdio: 'ignore' }).status === 0;
}
...
function fill(template, vars) {
  return String(template).replace(/\{(\w+)\}/g, (m, key) => Object.prototype.hasOwnProperty.call(vars, key) ? String(vars[key]) : m);
}
...
function buildPrompt(workflow, resolved, taskPrompt) {
  ...
  lines.push(
    ...
    '- Follow AGENTS.md and the repository guardrails.',
    '- Never weaken or bypass ai/guard.yaml, hooks, workflow gates, or eval rules.',
    '- Do not commit, push, deploy, publish, or write to external systems unless the workflow explicitly allows it and the guard approves it.',
    '- Return a concise, evidence-based final result. Do not claim checks you did not run.',
    '',
    'Task context:',
    taskPrompt.trim(),
  );
...
function validateWorkflow(name, workflow, agents) {
  ...
  for (const stage of workflow.stages || []) {
    ...
    if (ids.has(stage.id)) { problems.push(`${name}: duplicate stage "${stage.id}"`); }
    ids.add(stage.id);
  ...
  for (const stage of workflow.stages || []) {
    for (const dep of stage.needs || []) { if (!ids.has(dep)) { problems.push(`${name}/${stage.id}: unknown dependency "${dep}"`); } }
  }
  for (const [roleName, role] of Object.entries(workflow.roles || {})) {
    const candidates = roleCandidates(role);
    if (!candidates.length) { problems.push(`${name}/${roleName}: no executor`); }
...
function printWorkflow(workflow) {
  ...
  console.log(`orchestrator: ${workflow.orchestrator && workflow.orchestrator.executor || '–'}`);
  console.log('\nstages:');
  for (const stage of workflow.stages) {
    const who = stage.type === 'human_gate'
      ? 'human gate'
      : stage.role
        ? `${stage.role} → ${(workflow.roles[stage.role] || {}).executor || '–'}`
        : Array.isArray(stage.roles)
          ? `${stage.strategy || 'parallel'}: ${stage.roles.map(r => `${r}→${(workflow.roles[r] || {}).executor || '–'}`).join(', ')}`
          : `orchestrator → ${workflow.orchestrator && workflow.orchestrator.executor || '–'}`;
...
function execute(workflow, agents, roleName, args) {
  ...
  const argv = (resolved.agent.command || []).map(a => fill(a, {
    prompt,
    budget,
    cwd,
  ...
  const summary = {
    ...
    role: roleName,
    executor: resolved.agentName,
    command: argv.map(a => /\s/.test(a) ? JSON.stringify(a.length > 120 ? `${a.slice(0, 117)}…` : a) : a).join(' '),
  };
  if (args['dry-run']) {
    console.log(args.json ? JSON.stringify(summary, null, 2) : `${workflow.name}/${roleName} → ${resolved.agentName}\n${summary.command}`);
    return 0;
  }
  ...
  const res = spawnSync(argv[0], argv.slice(1), {
    ...
    env: {
      ...
      AI_WORKFLOW: workflow.name,
      AI_WORKFLOW_ROLE: roleName,
    },
    encoding: 'utf8',
    maxBuffer: 512 * 1024 * 1024,
    timeout: (Number(args['timeout-min']) || 60) * 60 * 1000,
    stdio: ['ignore', 'pipe', 'pipe'],
  });

  const answer = finalAnswer(resolved.agent, res.stdout || '', lastMessageFile);
  if (args['output-file']) {
    const output = path.resolve(args['output-file']);
    fs.mkdirSync(path.dirname(output), { recursive: true });
    fs.writeFileSync(output, answer.endsWith('\n') ? answer : `${answer}\n`);
  }
...
if (require.main === module) {
  ...
  const [cmd, workflowName, roleOrStage] = args._;
  let agents;
    ...
    switch (cmd) {
      case 'list':
        for (const name of listWorkflows()) {
          ...
          console.log(`${name.padEnd(16)} ${(data.stages || []).length} stages · ${Object.keys(data.roles || {}).length} roles · orchestrator ${data.orchestrator && data.orchestrator.executor || '–'}`);
        }
        break;

      case 'show': {
        if (!workflowName) { fail('usage: router.js show <workflow> [--json]'); }
        const { data } = loadWorkflow(workflowName);
        if (args.json) { console.log(JSON.stringify(data, null, 2)); }
        else { printWorkflow(data); }
        break;
      }
      ...
      case 'resolve': {
        ...
        const { data } = loadWorkflow(workflowName);
        let roleName = roleOrStage;
        const stage = stageFor(data, roleOrStage);
        if (stage) {
          if (stage.role) { roleName = stage.role; }
          else { fail(`stage "${roleOrStage}" is ${stage.type === 'human_gate' ? 'a human gate' : 'owned by the orchestrator or multiple roles'}; resolve a concrete role instead`); }
        }
        const r = resolveRole(data, agents, roleName, args.agent || null);
        const out = { workflow: data.name, role: roleName, executor: r.agentName, candidates: r.candidates, purpose: r.role.purpose || '', read_only: !!r.role.read_only, artifact: r.role.artifact || null };
        console.log(args.json ? JSON.stringify(out, null, 2) : `${out.workflow}/${out.role} → ${out.executor}${out.candidates.length > 1 ? ` (fallbacks: ${out.candidates.slice(1).join(', ')})` : ''}`);
        break;
      }

      case 'exec': {
        if (!workflowName || !roleOrStage) { fail('usage: router.js exec <workflow> <role|stage> --prompt-file FILE [--agent NAME] [--output-file FILE] [--dry-run]'); }
        const { data } = loadWorkflow(workflowName);
        let roleName = roleOrStage;
        const stage = stageFor(data, roleOrStage);
        if (stage) {
          if (!stage.role) { fail(`stage "${roleOrStage}" cannot be executed as one role; choose one of: ${(stage.roles || []).join(', ') || 'orchestrator/human'}`); }
          roleName = stage.role;
        }
        process.exitCode = execute(data, agents, roleName, args);
        break;
      }
      ...
      case 'check': {
        ...
        for (const name of listWorkflows()) {
          try {
            const { data } = loadWorkflow(name);
            all.push(...validateWorkflow(name, data, agents));
        ...
        if (all.length) {
          ...
          console.log(`✓ ${listWorkflows().length} workflow(s) valid · executors: ${Object.keys(agents).join(', ')}`);
        }
        break;
      }

      default:
        console.error('usage: router.js list | show <workflow> [--json] | resolve <workflow> <role|stage> [--agent NAME] [--json] | exec <workflow> <role|stage> --prompt-file FILE [--agent NAME] [--output-file FILE] [--budget USD] [--timeout-min M] [--cwd DIR] [--dry-run] | check');
        process.exitCode = 1;
    }
  } catch (e) {
    fail(e.message);
...
module.exports = { listWorkflows, loadWorkflow, loadAgents, resolveRole, validateWorkflow, buildPrompt };
