...
| | |
|---|---|
| **What it does** | A general coding agent (Claude Code, Codex, …) does plain tasks in this repo: fix a bug, add a line, answer a question, refuse to dump a secret. |
| **Entry point** | Headless CLI per agent, from `ai/agents.yaml`: `claude -p …`, `codex exec …` — the same prompt for every agent |
...
`.codex/hooks.json` (Codex), and `.husky/pre-commit`
